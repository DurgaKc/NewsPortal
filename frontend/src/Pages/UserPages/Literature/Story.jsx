...
const StoryList = () => {
  ...
  const { data: literatureResponse, isLoading } = useQuery({
    queryKey: ["literatureStories"],
    queryFn: async () => {
      const res = await getAllLiterature();
      // Filter only active stories
      return res.data.filter(
        (item) => item.category === "Story" && item.status === "active"
      );
    },
  });

  const stories = Array.isArray(literatureResponse) ? literatureResponse : [];
      ...
      <Box className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stories.map((story) => (
          <Card
            ...
            onClick={() => navigate(`/literature/${story._id}`)} // navigate to detail page
          >
            {/* Image */}
            {story.image ? (
              <img
                src={story.image ? `${backendUrl}/images/${story.image}` : "/placeholder.jpg"}
                alt={story.topic}
            ...
            <CardContent className="flex flex-col flex-grow p-4">
              ...
              <Typography className="text-gray-400 text-xs mt-1">
                प्रकाशित मिति: {new Date(story.date).toLocaleDateString("ne-NP")}
              </Typography>
              ...
              <Button
                ...
                onClick={() => navigate(`/literature/${story._id}`)}
                size="small"
              >
                थप पढ्नुहोस्
              </Button>
