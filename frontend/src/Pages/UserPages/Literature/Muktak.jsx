...
const MuktakList = () => {
  ...
  const { data: literatureResponse, isLoading } = useQuery({
    queryKey: ["literatureMuktak"],
    queryFn: async () => {
      const res = await getAllLiterature();
      // Filter only active muktaks
      return res.data.filter(
        (item) => item.category === "Muktak" && item.status === "active"
      );
    },
  });

  const muktaks = Array.isArray(literatureResponse) ? literatureResponse : [];
  ...
  if (!muktaks || muktaks.length === 0) {
    return (
      <Box className="flex justify-center items-center h-screen">
        <Typography variant="h6" className="text-gray-500">
          No active muktaks found
        </Typography>
      </Box>
    );
  }
      ...
      <Box className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {muktaks.map((muktak) => (
          <Card
            ...
            onClick={() => navigate(`/literature/${muktak._id}`)} // navigate to detail page
          >
            {/* Image */}
            {muktak.image ? (
              <img
                src={muktak.image ? `${backendUrl}/images/${muktak.image}` : "/placeholder.jpg"}
                alt={muktak.topic}
            ...
            <CardContent className="flex flex-col flex-grow p-4">
              ...
              <Typography className="text-gray-400 text-xs mt-1">
                प्रकाशित मिति: {new Date(muktak.date).toLocaleDateString("ne-NP")}
              </Typography>
              ...
              <Button
                ...
                onClick={() => navigate(`/literature/${muktak._id}`)}
                size="small"
              >
                थप पढ्नुहोस्
              </Button>
