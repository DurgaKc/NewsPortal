...
import { getNewsById, updateNews } from "./InterApi";
import toast from "react-hot-toast";
const backendUrl = import.meta.env.VITE_BACKEND_URL;

const EditInternational = ({ onClose, id }) => {
  ...
  const updateInternationalNews = useMutation({
    mutationFn: (updatedNews) => {
      const token = localStorage.getItem("token");
      return updateNews(id, updatedNews, token);
    },
    onSuccess: () => {
      toast.success("International News updated successfully!");
      refetch();
      onClose();
    },
    onError: (error) => {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    setLoading(true);
    updateInternationalNews.mutate(formData);
    setLoading(false);
  };
