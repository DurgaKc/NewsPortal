...
import { getEducationById, updateEducation } from "./EducationApi";
import toast from "react-hot-toast";

const backendUrl = import.meta.env.VITE_BACKEND_URL;

const EditEducation = ({ onClose, id }) => {
  ...
  // ✅ Mutation for updating education news
  const updateEducationNews = useMutation({
    mutationFn: (updatedNews) => {
      const token = localStorage.getItem("token");
      return updateEducation(id, updatedNews, token);
    },
    onSuccess: () => {
      toast.success("Education news updated successfully!");
      refetch();
      onClose();
    },
    onError: (error) => {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    setLoading(true);
    updateEducationNews.mutate(formData);
    setLoading(false);
  };
