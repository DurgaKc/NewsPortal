...
import { addLatestNews } from "./LatestNewsApi"; // <-- create this API

const AddLatestNews = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await addLatestNews(formData, token);

      toast.success("Latest News added successfully!");
      setFormData({
        topic: "",
        author: "", 
        description: "",
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to add latest news");
    } finally {
      setLoading(false);
    }
  };
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <Box className="flex justify-center gap-4 mt-6">
                ...
                >
                  {loading ? "Adding..." : "Add Latest News"}
                </Button>

                <Button
                  variant="contained"
                  onClick={() => navigate("/admin/latestnews")}
                  sx={{
                    textTransform: "none",
                    px: 4,
                    borderRadius: 2,
