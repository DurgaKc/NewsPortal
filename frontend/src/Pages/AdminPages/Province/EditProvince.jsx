...
import { getProvinceById, updateProvince } from "./ProvinceApi";
import toast from "react-hot-toast";

const backendUrl = import.meta.env.VITE_BACKEND_URL;

const EditProvince = ({ onClose, id }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await getProvinceById(id);
      return res;
    },
    enabled: !!id,
  });
  ...
  // ✅ Mutation for updating province news
  const updateProvinceNews = useMutation({
    mutationFn: (updatedNews) => {
      const token = localStorage.getItem("token");
      return updateProvince(id, updatedNews, token);
    },
    onSuccess: () => {
      toast.success("Province news updated successfully!");
      refetch();
      onClose();
    },
    onError: (error) => {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    setLoading(true);
    updateProvinceNews.mutate(formData);
    setLoading(false);
  };
