...
import { addProvince } from "./ProvinceApi";

const AddProvince = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!token) {
        toast.error("You must be logged in to add province news");
        setLoading(false);
        return;
      }

      await addProvince(formData, token);

      toast.success("Province news added successfully!");
      setFormData({
        topic: "",
        description: "",
        date: "",
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to add province news");
    } finally {
      setLoading(false);
    }
  };
