...
import { getSportsById } from "./SportsApi"; // Make sure this API exists
...
const SportsDetails = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await getSportsById(id);
        setNews(response);
      } catch (error) {
        console.error("Error fetching sports news detail:", error);
      } finally {
        setLoading(false);
      }
    };
