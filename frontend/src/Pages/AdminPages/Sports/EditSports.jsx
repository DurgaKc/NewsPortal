...
import { getSportsById, updateSports } from "./SportsApi";
import toast from "react-hot-toast";

const backendUrl = import.meta.env.VITE_BACKEND_URL;

const EditSports = ({ onClose, id }) => {
  ...
  const {
    ...
    queryKey: ["sports", id],
    queryFn: async () => {
      const res = await getSportsById(id);
      return res;
    },
    enabled: !!id,
  });
  ...
  // ✅ Mutation for updating sports news
  const updateSportsNews = useMutation({
    mutationFn: (updatedNews) => {
      const token = localStorage.getItem("token");
      return updateSports(id, updatedNews, token);
    },
    onSuccess: () => {
      toast.success("Sports news updated successfully!");
      refetch();
      onClose();
    },
    onError: (error) => {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    setLoading(true);
    updateSportsNews.mutate(formData);
    setLoading(false);
  };
