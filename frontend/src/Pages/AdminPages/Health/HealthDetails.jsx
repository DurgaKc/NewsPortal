...
import { getHealthById } from "./HealthApi"; // Make sure this API exists
...
const HealthDetails = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await getHealthById(id);
        setNews(response);
      } catch (error) {
        console.error("Error fetching health news detail:", error);
      } finally {
        setLoading(false);
      }
    };
