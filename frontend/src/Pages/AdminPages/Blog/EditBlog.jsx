...
import { getBlogById, updateBlog } from "./BlogApi";
import toast from "react-hot-toast";

const backendUrl = import.meta.env.VITE_BACKEND_URL;

const EditBlog = ({ onClose, id }) => {
  ...
  useEffect(() => {
    if (fetchedBlog) {
      setFormData({
        ...
        date: fetchedBlog.date ? fetchedBlog.date.split("T")[0] : "",
        video: fetchedBlog.video || null,
        status: fetchedBlog.status || "active",
      });

      // Show video preview if exists
      if (fetchedBlog.video) {
        setPreview(`${backendUrl}/videos/${fetchedBlog.video}`);
      }
    }
  }, [fetchedBlog]);
  ...
  const updateBlogData = useMutation({
    mutationFn: (updatedData) => {
      ...
      return updateBlog(id, updatedData, token);
    },
    onSuccess: () => {
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    setLoading(true);
    updateBlogData.mutate(formData);
    setLoading(false);
  };
